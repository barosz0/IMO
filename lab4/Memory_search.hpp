...
public:
    ...
    Memory_search(vector<vector<int>> &matrix, std::vector<int> &A, std::vector<int> &B);
...
void Memory_search::make_move(move_data move)
{
    ...
    if(move.type == OUTER_CHANGE)
    {
        ...
        // for(int i=0;i<A_cycle.size();i++){
        //     generate_moves_outer(i, 0);
        // }

        // for(int i=0;i<A_cycle.size();i++){
        //     generate_moves_outer(i, 1);
        // }
    }
    ...
    if(move.type == INNER_CHANGE)
    {
        ...
        bool a_order = check_order(index_a, move.neigh_A.second, cycle);
        bool b_order = check_order(index_b, move.neigh_B.second, cycle);
        // printf("1\n");
        
        if (index_a > index_b){

            // Sytuacja kiedy zamieniamy na skraju cyklu (pierwszy z ostatnim)
            // tymczasowe rozwiązanie może nie działać.
            // if(index_b == 0)
            // {
            //     printf("hej\n");
            //     index_b = cycle.size()-1;
            //     index_a -=1;
            //     for(int i = 0; i < cycle.size(); i++)
            //     {
            //         printf("%d ", cycle[i]);
        ...
        if(a_order)
        {
            // print("order: %d %d\n",check_order(index_a,move.neigh_A.second, cycle))
            // printf("No jestem\n");
            index_a -=1;
            index_b -=1;
        }
        ...
        // printf("3\n");
        if(index_a == -1)
            generate_moves_outer(cycle.size()-1, move.which_cycle);
        else
...
Memory_search::Memory_search(vector<vector<int>> &matrix_, std::vector<int> &A, std::vector<int> &B)
{
    matrix = matrix_;
    A_cycle = A;
    B_cycle = B;
