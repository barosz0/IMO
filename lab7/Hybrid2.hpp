#ifndef HYBRID_EVOLUTIONARY2
#define HYBRID_EVOLUTIONARY2
...
class Hybrid2 : public Local_search_abstract
{
private:
    std::random_device r;
    ...
    std::vector<int> longest_subset(const std::vector<int>& A, const std::vector<int>& B);
    ...
    void run() override;
public:
    Hybrid2(string filename_, int max_running_time_, bool do_local_search_, int population_size_);
    ~Hybrid2();
};

solution Hybrid2::find_best_solution()
{
...
bool Hybrid2::check_if_edge_in_cycle(int a, int b, vector<int> &cycle)
{
...
int Hybrid2::check_if_enter_population(solution &sol)
{
    int position = -1;
    for(int i=0; i<population.size();i++)
    {
...
std::vector<int> Hybrid2::longest_subset(const std::vector<int>& A, const std::vector<int>& B) {
    // std::vector<int> A = A_;
    // A.insert(A.end(), A_.begin(), A_.end());
    int n = A.size();
    int m = B.size();
...
solution Hybrid2::combine_solutions(solution &A, solution &B)
{

    // pierwsza opcja polaczenia
    vector<int> new_A1 = longest_subset(A.A_cycle, B.A_cycle);
    vector<int> new_B1 = longest_subset(A.B_cycle, B.B_cycle);

    // druga opcja polaczenia
    vector<int> new_A2 = longest_subset(A.A_cycle, B.B_cycle);
    vector<int> new_B2 = longest_subset(A.B_cycle, B.A_cycle);
    

    vector<int> new_A_cycle;
    vector<int> new_B_cycle;

    if(new_A1.size() + new_B1.size() < new_A2.size() + new_B2.size())
    {
        new_A_cycle = new_A2;
        new_B_cycle = new_B2;
    }
    ...
    // vector<int> new_A_cycle = longest_subset(A.A_cycle, B.A_cycle);
    // vector<int> new_B_cycle = longest_subset(A.B_cycle, B.B_cycle);
...
solution Hybrid2::make_new_solution()
{
    ...
    return combine_solutions(population[a],population[b]);
...
void Hybrid2::fill_starting_population(int n)
{
    while(population.size() < n)
    {   
        Greedy_2regret_cycle cycle(filename);
        population.push_back(solution(&cycle));
...
bool Hybrid2::check_if_score_in_population(int score)
{
    for(int i = 0; i<population.size(); i++)
    {
...
Hybrid2::Hybrid2(string filename_, int max_running_time_, bool do_local_search_ = true, int population_size_ = 20)
{
...
// zmiana combinacji na najdluzszy wspolny podciag
// zmiana generowania poczacztkowj populacji aby dzialala szybciej (zamiana na 2regret i test czy to działa)