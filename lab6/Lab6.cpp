...
int count_same_vertex(vector<int> A, vector<int> B)
{
    int sum = 0;
...
int count_first_score(Memory_search &A, Memory_search &B)
{
    int sum1 = count_same_vertex(A.get_A_cycle(), B.get_A_cycle()) + count_same_vertex(A.get_B_cycle(), B.get_B_cycle());
    int sum2 = count_same_vertex(A.get_A_cycle(), B.get_B_cycle()) + count_same_vertex(A.get_B_cycle(), B.get_A_cycle());
...
int calc_second_score(Memory_search &A, Memory_search &B)
{
    int sum = 0;
    vector<int> A1 = A.get_A_cycle();
    vector<int> A2 = B.get_A_cycle();
    vector<int> B1 = A.get_B_cycle();
    vector<int> B2 = B.get_B_cycle();

    for(int i=0; i<A1.size()-1; i++)
    {
        sum += check_if_edge_in_cycle(A1[i],A1[i+1], A2);
        sum += check_if_edge_in_cycle(A1[i],A1[i+1], B2);

        sum += check_if_edge_in_cycle(B1[i],B1[i+1], A2);
        sum += check_if_edge_in_cycle(B1[i],B1[i+1], B2);
    }
    sum += check_if_edge_in_cycle(A1[A1.size()-1],A1[0], A2);
    sum += check_if_edge_in_cycle(A1[A1.size()-1],A1[0], B2);

    sum += check_if_edge_in_cycle(B1[B1.size()-1],B1[0], A2);
    sum += check_if_edge_in_cycle(B1[B1.size()-1],B1[0], B2);

    return sum;
}

void do_test(int n_points, string filename = "Output/test_output.txt")
{
    vector<Memory_search> list;
    for(int i=0; i<n_points; i++)
    {
        Random_cycle cycle("Data/kroB200.tsp", 0, i);
        Memory_search MS(&cycle);
        list.push_back(MS);
    }
    int index = 0;
    ...
    {
        if(list[i].get_length() < list[index].get_length())
            index = i;
    }

    vector<int> first_scores;
    vector<int> second_scores;

    for(int i=0; i<list.size();i++)
    {
        first_scores.push_back(count_first_score(list[i], list[index]));
        second_scores.push_back(calc_second_score(list[i], list[index]));
        // printf("Score: %d\n", second_scores[i]);
    }

    vector<double> avg_first_scores;
    vector<double> avg_second_scores;
    ...
    for(int j=0; j<list.size();j++)
    {
        ...
        {
            if(i==j) continue;
            first_avg += count_first_score(list[i], list[index]);
            second_avg += calc_second_score(list[i], list[index]);
        }

        avg_first_scores.push_back(first_avg/(double)list.size());
        avg_second_scores.push_back(second_avg/(double)list.size());
    }
    std::ofstream plik(filename);
    for(int i=0; i < list.size();i++)
    {
        plik << list[i].get_length() << ";" << first_scores[i] << ";" << second_scores[i] << ";" << avg_first_scores[i] << ";" << avg_second_scores[i] << "\n";
