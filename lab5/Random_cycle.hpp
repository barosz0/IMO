#ifndef RANDOM_CYCLE_DEFINE
#define RANDOM_CYCLE_DEFINE
...
public:
    ...
    Random_cycle(string file, int a, unsigned seed);
    ~Random_cycle();
};
...
Random_cycle::Random_cycle(string file, int number) : Cycle_abstract(file, number)
{
    seed = std::chrono::system_clock::now().time_since_epoch().count();
    run();
    //cout << "Random cycle length: "<< this->get_length()<<endl;
}
Random_cycle::Random_cycle(string file, int number, unsigned seed_) : Cycle_abstract(file, number)
{
    seed = seed_;
    run();
    //cout << "Random cycle length: "<< this->get_length()<<endl;
