...
Cycle_abstract::Cycle_abstract(vector<vector<int>> matrix_, vector<pair<double, double>> coords_, std::vector<int> A, std::vector<int> B)
{
    ...
    if(A_cycle.size() == 0 || B_cycle.size() == 0)
    {
        srand((unsigned) time(NULL));
        int random_index = rand() % coords.size();

        if(A_cycle.size() == 0)
        {
            while(taken[random_index]) random_index = rand() % coords.size();
            A_cycle.push_back(random_index);
            taken[random_index] = true;
        }
        if(B_cycle.size() == 0)
        {
            while(taken[random_index]) random_index = rand() % coords.size();
            B_cycle.push_back(random_index);
            taken[random_index] = true;
        }
    }
